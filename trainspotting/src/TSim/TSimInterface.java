...
public final class TSimInterface {
	 ...
	 *
	 * Further we maintain a vector of TrainEvents indexed by trainId,
	 * where a non-null entry means a fatal event has occurred.
	 */

	private LinkedBlockingQueue<CommandStatus> commandFIFO = new LinkedBlockingQueue<>();

	private AddingArrayList<LinkedBlockingQueue<SensorEvent>> sensorVec = new AddingArrayList<>();

	private AddingArrayList<TrainEvent> trainVec = new AddingArrayList<>();
		...
		while (true) {
			...
			try {
				...
				if (dInfo instanceof CommandStatus)
					...
					TrainEvent tEvent = (TrainEvent) dInfo;
	...
	private void reportTrainEvent(TrainEvent e) {
		this.err.println(e);
	}
	...
	private LinkedBlockingQueue<SensorEvent> getSensorEventQueue(int trainId) {
		...
		if (trainSensorFIFO == null) {
			synchronized (this) {
				trainSensorFIFO = sensorVec.get(trainId);

				if (trainSensorFIFO == null) {
					trainSensorFIFO = new LinkedBlockingQueue<>();
	...
	public synchronized void setSpeed(int trainId, int speed) throws CommandException {
		TrainEvent tEvent = trainVec.get(trainId);
